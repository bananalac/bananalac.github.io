...
$(document).ready(function() {
    ...
    if(!userId) {
        ...
        $(".pageTitle").html(`${userInfo.blName}'s Profile`)
        $("#profileName").html(userInfo.name);
        $("#profileRole").html(userInfo.role);
        $("#profilePicture").attr("src", userInfo.pic);
        $("#profilePhotos").html(`<strong>${userInfo.images.length}</strong>Photos`);
        $("#profileTools").html(`<strong>3</strong>Tools`);
        $("#profileBlogs").html(`<strong>${blogPosts.length}</strong>Blogposts`);
        $("#profileGithubFollowers").html(`<strong>333</strong>Github`);
        $("#profileBio").html(userInfo.bio);
